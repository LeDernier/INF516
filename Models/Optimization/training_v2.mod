...
set T_1 := 1..n1;           # here we use all the available data for training
set T_2 := 1..n2;

# variable values
param x1{i in T_1};			# input values (first component)
param x2{j in T_2};			# input values (second component)

# function values
param f{i in T_1, j in T_2};	# output values assuming f is scalar
...
# big M
param M default 10e6;

#### decision variables ####
var v_plus{T,V} >= 0;					# positive post-activated values
var v_minus{T,V} >= 0;					# negative post-activated values
#var z{T,V} >=0, <=1, default 1;			# relaxed binary variable to have v_plus <= 0 xor v_minus <= 0	
var z{T,V} binary, default 1;
var A{G};								# weights NN
...
#### objective function ####
minimize nnerr:
	sum{(i_x1,i_x2) in T, o in O} (v_plus[i_x1,i_x2,o] - v_minus[i_x1,i_x2,o] - f[i_x1,i_x2])^2;

#### constraints ####
# input
subject to input_x1_upper{(i_x1,i_x2) in T}:
	v_plus[i_x1,i_x2,1] - v_minus[i_x1,i_x2,1] <= x1[i_x1];
subject to input_x1_lower{(i_x1,i_x2) in T}:
	v_plus[i_x1,i_x2,1] - v_minus[i_x1,i_x2,1] >= x1[i_x1];
subject to input_x2_upper{(i_x1,i_x2) in T}:
	v_plus[i_x1,i_x2,2] - v_minus[i_x1,i_x2,2] <= x2[i_x2];
subject to input_x2_lower{(i_x1,i_x2) in T}:
	v_plus[i_x1,i_x2,2] - v_minus[i_x1,i_x2,2] >= x2[i_x2];
